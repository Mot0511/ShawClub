...
$DBdata = [file_get_contents('data/hostDB.txt'), file_get_contents('data/loginDB.txt'), file_get_contents('data/passwordDB.txt'), file_get_contents('data/nameDB.txt')];
  ...
  if ($status == 2){
    ...
    mysqli_query($link, "INSERT INTO point SET products = '".$products."', price = ".$price.", number = '".$number."', email = '".$email."', status = '".$status."', address = '".$address."', point = '".$point."', carrier = '".$data[0]['carrier']."'");
  }
  else{
